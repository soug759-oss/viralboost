...
const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY || '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' });
...
// ── UPLOAD FICHIERS (images/vidéos) — Cloudinary ──
...
// Config Cloudinary
cloudinary.config({
  cloud_name: process.env.CLOUDINARY_CLOUD_NAME || 'GIORGIO',
  api_key:    process.env.CLOUDINARY_API_KEY    || '243731968826348',
  api_secret: process.env.CLOUDINARY_API_SECRET || 'nAjJsBSwhpI3NXvTAaTRQq2gevU',
});

// Multer en mémoire (pas de disque → compatible Render)
const upload = multer({
...
function uploadToCloudinary(buffer, mimetype, folder = 'viralboost') {
